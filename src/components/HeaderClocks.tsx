...
import { RotateCcw, Maximize2, Minimize2, Info, Rocket, Swords } from 'lucide-react';
...
export const HeaderClocks: React.FC<HeaderClocksProps> = ({
  ...
  return (
    <header className="w-full bg-black border-b border-white/15 px-2 py-2 sm:px-4 sm:py-2.5 flex items-center justify-between z-30 text-white font-sans select-none shrink-0 shadow-2xl relative">
      {/* Left Action Buttons: Spaceship Hangar & Combat Deck */}
      <div className="flex items-center gap-1.5 sm:gap-2 shrink-0">
        {onOpenHangar && (
          <button
            onClick={onOpenHangar}
            className="flex items-center gap-1.5 px-2.5 py-1 sm:px-3 sm:py-1.5 rounded-sm bg-cyan-500/15 hover:bg-cyan-500/30 text-cyan-300 border border-cyan-400/50 hover:border-cyan-400 transition-all text-[10px] sm:text-[11px] uppercase tracking-wider font-bold font-mono shadow-[0_0_12px_rgba(0,242,254,0.2)]"
            title="Open Spaceship Fleet Hangar"
          >
            <Rocket className="w-3.5 h-3.5 text-cyan-400" />
            <span className="hidden sm:inline">Ship Hangar</span>
          </button>
        )}
        ...
        {onToggleCombatDeck && (
          <button
            ...
            className={`flex items-center gap-1.5 px-2.5 py-1 sm:px-3 sm:py-1.5 rounded-sm border transition-all text-[10px] sm:text-[11px] uppercase tracking-wider font-bold font-mono ${
              isCombatDeckOpen
            ...
            title="Toggle Arcade Combat Deck"
          >
            <Swords className="w-3.5 h-3.5 text-amber-400" />
            <span className="hidden sm:inline">Combat Deck</span>
          </button>
        )}
      </div>
