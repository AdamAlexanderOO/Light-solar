...
import { SpaceshipModel, FlightTelemetry, VisionFusionMode, CombatTarget, Projectile, ExplosionParticle } from '../types/spaceship';
import { buildProceduralSpaceship, buildCombatTargetMesh, BuiltSpaceship } from '../utils/spaceship3D';
...
interface Earth3DProps {
  ...
  cameraMode: 'free' | 'follow-shadow' | 'focused-station' | 'top-down' | 'spain-fixed' | 'chase-ship' | 'cockpit-ship';
  ...
  // Spaceship & Combat Props
  selectedShip: SpaceshipModel;
  visionMode: VisionFusionMode;
  flightTelemetry: FlightTelemetry;
  onUpdateFlightTelemetry?: (updater: (prev: FlightTelemetry) => FlightTelemetry) => void;
  combatTargets: CombatTarget[];
  onTargetHit?: (targetId: string, damage: number) => void;
  fireTrigger?: number;
  secondaryFireTrigger?: number;
  boostTrigger?: number;
...
export const Earth3D: React.FC<Earth3DProps> = ({
  ...
  visionMode,
  flightTelemetry,
  onUpdateFlightTelemetry,
  combatTargets,
  onTargetHit,
  fireTrigger = 0,
  secondaryFireTrigger = 0,
  boostTrigger = 0,
  shieldTrigger = 0,
  specialTrigger = 0,
  lockedTarget = null,
  ...
  const onUpdateFlightTelemetryRef = useRef(onUpdateFlightTelemetry);
  const onTargetHitRef = useRef(onTargetHit);
  const flightTelemetryRef = useRef(flightTelemetry);
  flightTelemetryRef.current = flightTelemetry;
  ...
  useEffect(() => {
    ...
    const earthMat = new THREE.ShaderMaterial({
      ...
      fragmentShader: `
        ...
        void main() {
          ...
          // 5. Light Vision Fusion Multi-Spectral Optic Filters
          // u_vision_mode: 0=standard, 1=fusion-corona, 2=thermal-infrared, 3=cyber-radar, 4=ultraviolet-aurora
          if (u_vision_mode > 0.5 && u_vision_mode < 1.5) {
            // Corona / Solar Flare HDR Fusion: hyper-boost solar glint, golden corona rim, totality halo
            float coronaBoost = pow(max(0.0, ndotl), 2.2) * 1.6;
            vec3 coronaTint = vec3(1.0, 0.62, 0.18);
            finalColor = finalColor * vec3(1.15, 0.95, 0.8) + coronaTint * coronaBoost * 0.5;
            if (u_has_umbra > 0.5) {
              finalColor += vec3(1.0, 0.75, 0.2) * umbraRingGlow * 3.2;
            }
          } else if (u_vision_mode > 1.5 && u_vision_mode < 2.5) {
            // Thermal Infrared FLIR Heat-Vision: false color heat map
            float heat = clamp(ndotl * 0.65 + 0.35, 0.0, 1.0);
            heat += length(nightColor) * 0.35; // City thermal heat signatures
            heat -= shadowDarkness * 0.45; // Shadow cooling
            heat = clamp(heat, 0.0, 1.0);
            vec3 flirColor;
            if (heat < 0.25) {
              flirColor = mix(vec3(0.04, 0.02, 0.18), vec3(0.18, 0.05, 0.55), heat / 0.25);
            } else if (heat < 0.5) {
              flirColor = mix(vec3(0.18, 0.05, 0.55), vec3(0.85, 0.1, 0.25), (heat - 0.25) / 0.25);
            } else if (heat < 0.75) {
              flirColor = mix(vec3(0.85, 0.1, 0.25), vec3(0.98, 0.65, 0.05), (heat - 0.5) / 0.25);
            } else {
              flirColor = mix(vec3(0.98, 0.65, 0.05), vec3(1.0, 1.0, 0.95), (heat - 0.75) / 0.25);
            }
            finalColor = mix(finalColor * 0.15, flirColor, 0.88);
          } else if (u_vision_mode > 2.5 && u_vision_mode < 3.5) {
            // Cyber Quantum Radar: emerald & cyan tactical HUD grid with scanlines
            float lumaVal = dot(finalColor, vec3(0.299, 0.587, 0.114));
            vec3 cyberGreen = vec3(0.05, 0.98, 0.65);
            vec3 cyberCyan = vec3(0.0, 0.68, 0.98);
            float gridLat = abs(sin(vUv.y * 72.0 * 3.14159));
            float gridLon = abs(sin(vUv.x * 72.0 * 3.14159));
            float grid = smoothstep(0.92, 0.98, max(gridLat, gridLon));
            float scanline = sin(vUv.y * 300.0 - u_time * 5.0) * 0.08;
            vec3 mono = mix(cyberCyan * 0.2, cyberGreen, lumaVal * 1.3);
            finalColor = mono + cyberGreen * grid * 0.4 + vec3(scanline);
          } else if (u_vision_mode > 3.5) {
            // Ultraviolet Magneto-Flux & Aurora Borealis
            float latAbs = abs(vPositionWorld.y);
            float auroraBand = smoothstep(0.60, 0.95, latAbs) * (0.65 + 0.35 * sin(vUv.x * 45.0 + u_time * 2.5));
            vec3 auroraColor = vec3(0.45, 0.15, 0.95) * 1.3 + vec3(0.1, 0.95, 0.65) * sin(vUv.x * 24.0 + u_time * 1.8);
            finalColor = finalColor * vec3(0.75, 0.82, 1.15) + auroraColor * auroraBand * 0.9;
          }
    ...
    spaceshipGroupRef.current = spaceshipGroup;

    // 7. Orbital Combat Targets Group
    const targetsGroup = new THREE.Group();
    scene.add(targetsGroup);
    targetsGroupRef.current = targetsGroup;

    // 8. Laser & Torpedo Projectiles Group
    const projectilesGroup = new THREE.Group();
    scene.add(projectilesGroup);
    projectilesGroupRef.current = projectilesGroup;

    // 9. Explosion Spark Particles Group
    ...
    const animate = () => {
      ...
      // Update shader time for UV Aurora & Cyber Radar dynamic waves
      if (earthMeshRef.current && earthMeshRef.current.material instanceof THREE.ShaderMaterial) {
        earthMeshRef.current.material.uniforms.u_time.value = now * 0.001;
      }

      // Smooth camera spherical transition if target is set (prevents excessive spinning or diving through Earth)
      if (targetCamPosRef.current && cameraRef.current && controlsRef.current && cameraMode !== 'chase-ship' && cameraMode !== 'cockpit-ship') {
      ...
      // --- Spaceship Flight & Orbital Kinematics ---
      const shipState = shipPosStateRef.current;
      const curTelemetry = flightTelemetryRef.current;
      const isBoosting = now < shipState.boostEndTime;
      const isShieldActive = now < shipState.shieldEndTime;

      if (curTelemetry.flightMode === 'auto-patrol') {
        // Auto-Patrol along the 2026 Solar Eclipse Path
        // Lead the umbra shadow slightly ahead so the pilot observes totality sweeps
        const umbraPos = getUmbraPosition(currentTimestamp + 60);
        if (umbraPos) {
          shipState.lat = THREE.MathUtils.lerp(shipState.lat, umbraPos.lat, 0.05);
          shipState.lon = THREE.MathUtils.lerp(shipState.lon, umbraPos.lon, 0.05);
          shipState.heading = THREE.MathUtils.lerp(shipState.heading, 118, 0.05); // Northwest to Southeast trajectory
        }
      } else if (curTelemetry.flightMode === 'manual-pilot') {
        // Manual Arcade Pilot Controls via Keys Pressed
        const keys = keysPressedRef.current;
        const turnSpeed = (selectedShip.stats.handling / 100) * 80 * dt;
        const speedMultiplier = isBoosting ? 1.9 : 1.0;
        const currentMach = selectedShip.stats.topSpeedMach * shipState.throttle * speedMultiplier;

        if (keys['KeyA'] || keys['ArrowLeft']) {
          shipState.heading = (shipState.heading - turnSpeed + 360) % 360;
          shipState.roll = THREE.MathUtils.lerp(shipState.roll, -0.4, 0.1);
        } else if (keys['KeyD'] || keys['ArrowRight']) {
          shipState.heading = (shipState.heading + turnSpeed) % 360;
          shipState.roll = THREE.MathUtils.lerp(shipState.roll, 0.4, 0.1);
        } else {
          shipState.roll = THREE.MathUtils.lerp(shipState.roll, 0, 0.1);
        }

        if (keys['KeyW'] || keys['ArrowUp']) {
          shipState.throttle = Math.min(1.0, shipState.throttle + 0.5 * dt);
          shipState.pitch = THREE.MathUtils.lerp(shipState.pitch, -0.2, 0.1);
        } else if (keys['KeyS'] || keys['ArrowDown']) {
          shipState.throttle = Math.max(0.2, shipState.throttle - 0.5 * dt);
          shipState.pitch = THREE.MathUtils.lerp(shipState.pitch, 0.2, 0.1);
        } else {
          shipState.pitch = THREE.MathUtils.lerp(shipState.pitch, 0, 0.1);
        }

        // Advance ship lat/lon based on heading and speed
        const headingRad = THREE.MathUtils.degToRad(shipState.heading);
        const distStep = (currentMach * 0.008) * dt;
        shipState.lat += Math.cos(headingRad) * distStep * 10;
        shipState.lon += Math.sin(headingRad) * distStep * 10;

        // Spherical boundary clamping
        if (shipState.lat > 85) shipState.lat = 85;
        if (shipState.lat < -85) shipState.lat = -85;
        if (shipState.lon > 180) shipState.lon -= 360;
        if (shipState.lon < -180) shipState.lon += 360;

        shipState.speedMach = currentMach;
      }

      // Update 3D Spaceship Position & Orientation in scene
      if (spaceshipGroupRef.current) {
        const orbitRadius = EARTH_RADIUS + (shipState.altitude * 0.1);
        const shipVec = latLonToVector3(shipState.lat, shipState.lon, orbitRadius);
        spaceshipGroupRef.current.position.set(...shipVec);

        // Calculate surface normal and tangent forward vector
        const normal = new THREE.Vector3(...shipVec).normalize();
        const headingRad = THREE.MathUtils.degToRad(shipState.heading);

        // Basis vectors on sphere tangent plane
        ...
        // Construct orientation matrix from forward, normal (up), and right
        const m = new THREE.Matrix4().makeBasis(right, normal, forward.clone().negate());
        spaceshipGroupRef.current.quaternion.setFromRotationMatrix(m);

        // Apply dynamic banking roll
        spaceshipGroupRef.current.rotateZ(shipState.roll);
        spaceshipGroupRef.current.rotateX(shipState.pitch);

        // Animate Thruster Flames
        if (builtShipRef.current) {
          const flameScale = (isBoosting ? 2.5 : 1.0 + (shipState.throttle * 0.8)) + Math.sin(now * 0.03) * 0.2;
          builtShipRef.current.thrusterFlames.forEach((flame) => {
            flame.scale.set(1, 1, flameScale);
          });

          // Animate Shield Bubble
          if (builtShipRef.current.shieldMesh) {
            builtShipRef.current.shieldMesh.visible = isShieldActive;
            if (isShieldActive) {
              const mat = builtShipRef.current.shieldMesh.material as THREE.MeshBasicMaterial;
              mat.opacity = 0.35 + Math.sin(now * 0.01) * 0.2;
            }
          }
        }

        // Camera Follow Modes (Chase & Cockpit)
        if (cameraRef.current) {
          if (cameraMode === 'chase-ship') {
            controls.enabled = false;
            const shipPos = spaceshipGroupRef.current.position;
            const chaseOffset = normal.clone().multiplyScalar(5).sub(forward.clone().multiplyScalar(15));
            const targetCamPos = shipPos.clone().add(chaseOffset);
            cameraRef.current.position.lerp(targetCamPos, 0.15);
            cameraRef.current.lookAt(shipPos.clone().add(forward.clone().multiplyScalar(10)));
          } else if (cameraMode === 'cockpit-ship') {
            controls.enabled = false;
            const shipPos = spaceshipGroupRef.current.position;
            const cockpitOffset = forward.clone().multiplyScalar(2.2).add(normal.clone().multiplyScalar(0.8));
            cameraRef.current.position.copy(shipPos.clone().add(cockpitOffset));
            cameraRef.current.lookAt(shipPos.clone().add(forward.clone().multiplyScalar(30)));
          } else {
      ...
      // --- Projectiles Update & Collision Detection ---
      const activeProj = activeProjectilesRef.current;
      for (let i = activeProj.length - 1; i >= 0; i--) {
        ...
        let hit = false;
        // Check collision against active targets
        if (targetsGroupRef.current) {
          for (let j = 0; j < targetsGroupRef.current.children.length; j++) {
            const targetObj = targetsGroupRef.current.children[j];
            const dist = p.mesh.position.distanceTo(targetObj.position);
            if (dist < 5.5) {
              hit = true;
              const targetId = targetObj.userData.id;
              onTargetHitRef.current?.(targetId, p.damage);
              ...
              if (particlesGroupRef.current) {
                for (let k = 0; k < 18; k++) {
                  ...
                  const pMat = new THREE.MeshBasicMaterial({ color: 0x00f2fe, transparent: true, opacity: 1.0 });
                  const pMesh = new THREE.Mesh(pGeo, pMat);
                  pMesh.position.copy(targetObj.position);
      ...
      // --- Particles Update ---
      const activeParts = activeParticlesRef.current;
      for (let i = activeParts.length - 1; i >= 0; i--) {
        ...
        mat.opacity = Math.max(0, part.life / part.maxLife);
        part.mesh.scale.multiplyScalar(0.96);

        if (part.life <= 0) {
          particlesGroupRef.current?.remove(part.mesh);
          activeParts.splice(i, 1);
        }
      }

      // Rotate Combat Targets slowly in orbit
      if (targetsGroupRef.current) {
        targetsGroupRef.current.children.forEach((t) => {
          t.rotation.y += 0.01;
          t.rotation.x += 0.005;
        });
      }

      // Periodic Telemetry sync back to state
      if (Math.random() < 0.2 && onUpdateFlightTelemetryRef.current) {
        onUpdateFlightTelemetryRef.current((prev) => ({
          ...prev,
          lat: shipState.lat,
          lon: shipState.lon,
          altitudeKm: shipState.altitude,
          speedMach: shipState.speedMach,
          headingDeg: shipState.heading,
          overheat: Math.max(0, prev.overheat - dt * 25),
          energy: Math.min(100, prev.energy + dt * 10)
        }));
      }
      ...
      if (cameraMode !== 'chase-ship' && cameraMode !== 'cockpit-ship') {
        controls.update();
      }
  ...
  useEffect(() => {
    ...
    if (earthMeshRef.current && earthMeshRef.current.material instanceof THREE.ShaderMaterial) {
      ...
      // Update Vision Fusion Mode
      const modeValue = visionMode === 'standard' ? 0.0 :
                        visionMode === 'fusion-corona' ? 1.0 :
                        visionMode === 'thermal-infrared' ? 2.0 :
                        visionMode === 'cyber-radar' ? 3.0 : 4.0;
      earthMeshRef.current.material.uniforms.u_vision_mode.value = modeValue;
  ...
  }, [currentTimestamp, selectedStation, cameraMode, cameraResetTrigger, showPathLine, showPenumbra, showDayNightTerminator, showCelestialIcons, visionMode]);

  // Synchronize 3D Spaceship Model
  useEffect(() => {
    if (!sceneRef.current || !spaceshipGroupRef.current) return;
    const group = spaceshipGroupRef.current;
    while (group.children.length > 0) {
      group.remove(group.children[0]);
    }
    const built = buildProceduralSpaceship(selectedShip);
    builtShipRef.current = built;
    group.add(built.group);
  }, [selectedShip]);

  // Synchronize Orbital Combat Targets
  useEffect(() => {
    if (!sceneRef.current || !targetsGroupRef.current) return;
    const group = targetsGroupRef.current;
    while (group.children.length > 0) {
      group.remove(group.children[0]);
    }
    combatTargets.forEach((target) => {
      if (!target.isDestroyed) {
        const mesh = buildCombatTargetMesh(target);
        group.add(mesh);
      }
    });
  }, [combatTargets]);
  ...
  const fireWeapons = useCallback((isSecondary: boolean) => {
    if (!spaceshipGroupRef.current || !builtShipRef.current || !projectilesGroupRef.current) return;
    const ship = spaceshipGroupRef.current;
    const built = builtShipRef.current;

    const forward = new THREE.Vector3(0, 0, -1).applyQuaternion(ship.quaternion).normalize();
    const colorHex = isSecondary ? 0xf59e0b : (selectedShip.primaryWeapon.type === 'ion' ? 0xa855f7 : selectedShip.primaryWeapon.type === 'plasma' ? 0xef4444 : 0x00f2fe);
    const muzzles = isSecondary ? [new THREE.Vector3(0, 0, 0)] : built.muzzlePoints;

    muzzles.forEach((muzzleLocal) => {
      const muzzleWorld = muzzleLocal.clone().applyQuaternion(ship.quaternion).add(ship.position);
      const points = [
        muzzleWorld.clone(),
        muzzleWorld.clone().addScaledVector(forward, isSecondary ? 3.0 : 2.2)
      ];
      const geo = new THREE.BufferGeometry().setFromPoints(points);
      const mat = new THREE.LineBasicMaterial({
        color: colorHex,
        linewidth: isSecondary ? 4 : 2,
        transparent: true,
        opacity: 0.95
      });
      const line = new THREE.Line(geo, mat);
      projectilesGroupRef.current?.add(line);

      activeProjectilesRef.current.push({
        mesh: line,
        dir: forward.clone(),
        speed: isSecondary ? 2.2 : 3.4,
        traveled: 0,
        maxDist: 200,
        damage: isSecondary ? selectedShip.secondaryWeapon.damage : selectedShip.primaryWeapon.damage,
        color: isSecondary ? '#f59e0b' : '#00f2fe'
      });
    });

    if (onUpdateFlightTelemetryRef.current) {
      onUpdateFlightTelemetryRef.current((prev) => ({
        ...prev,
        overheat: Math.min(100, prev.overheat + (isSecondary ? 22 : 8))
      }));
    }
    ...
    if (isSecondary) {
      ...
      audioSynth.playLaser(selectedShip.primaryWeapon.type);
    }
  }, [selectedShip]);

  // Weapon Trigger Handlers
  useEffect(() => {
    if (fireTrigger <= 0) return;
    fireWeapons(false);
  }, [fireTrigger, fireWeapons]);

  useEffect(() => {
    if (secondaryFireTrigger <= 0) return;
    fireWeapons(true);
  }, [secondaryFireTrigger, fireWeapons]);

  useEffect(() => {
    if (boostTrigger <= 0) return;
    shipPosStateRef.current.boostEndTime = performance.now() + 2500;
    audioSynth.playBoost();
  }, [boostTrigger]);

  useEffect(() => {
    if (shieldTrigger <= 0) return;
    shipPosStateRef.current.shieldEndTime = performance.now() + 3000;
    audioSynth.playShield();
  }, [shieldTrigger]);

  // Keyboard Event Listeners for Manual Piloting and Combat
  useEffect(() => {
    const handleKeyDown = (e: KeyboardEvent) => {
      ...
      if (e.code === 'Space' || e.code === 'KeyF') {
        ...
        e.preventDefault();
        onCameraModeChange(cameraMode === 'chase-ship' ? 'cockpit-ship' : cameraMode === 'cockpit-ship' ? 'free' : 'chase-ship');
      }
    };
  ...
  }, [fireWeapons, cameraMode, onCameraModeChange]);
  ...
  return (
    <div className="relative w-full h-full min-h-[400px] flex-1 bg-transparent overflow-hidden rounded-xl border border-white/15 shadow-2xl">
      ...
      {/* Cockpit / Chase Tactical HUD Overlay */}
      {(cameraMode === 'chase-ship' || cameraMode === 'cockpit-ship') && (
        <CockpitHudOverlay
          ...
          onFirePrimary={() => fireWeapons(false)}
          onFireSecondary={() => fireWeapons(true)}
          onBoost={() => {
            shipPosStateRef.current.boostEndTime = performance.now() + 2500;
            audioSynth.playBoost();
          }}
          onToggleCamera={() => {
            onCameraModeChange(cameraMode === 'chase-ship' ? 'cockpit-ship' : 'chase-ship');
          }}
        />
      )}
    </div>
